...
{
    internal static class HornetFfbDeviceProbe
    {
        ...
        internal static string GetLaunchSelection(string value)
        {
            ...
            return Regex.IsMatch(value, @"\A(?:wheel|gamepad):[0-9]{1,5}\z")
                ? value : "off";
        }
        ...
        public static List<DynamicDropdownOption> GetDevices()
        {
            ...
            var hornetDirectory = Path.Combine(Directory.GetCurrentDirectory(), "TeknoHornet");
            var probePath = Path.Combine(hornetDirectory, "HornetHaptic.exe");
            ...
            try
            {
                using (var process = new Process
                {
                    StartInfo = new ProcessStartInfo(probePath, "--list")
                    {
                        ...
                        WorkingDirectory = hornetDirectory,
            ...
            {
                // Older TeknoHornet packages may not include the optional haptic probe.
            }
