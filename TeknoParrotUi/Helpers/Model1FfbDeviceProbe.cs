...
{
    internal static class Model1FfbDeviceProbe
    {
        ...
        public static List<DynamicDropdownOption> GetDevices()
        {
            ...
            var model1Directory = Path.Combine(Directory.GetCurrentDirectory(), "TeknoModel1");
            var probePath = new[]
            {
                Path.Combine(model1Directory, "model1haptic.exe"),
                Path.Combine(model1Directory, "tools", "model1haptic.exe")
            ...
            try
            {
                using (var process = new Process
                {
                    StartInfo = new ProcessStartInfo(probePath, "--list")
                    {
                        ...
                        WorkingDirectory = model1Directory,
            ...
            {
                // Older Model 1 packages may not include the optional haptic probe.
            }
