    ...
    public partial class GameRunning : Window
    {
        ...
        public GameRunning(GameProfile gameProfile, string loaderExe, string loaderDll, bool isTest, bool runEmuOnly = false, bool profileLaunch = false, Library library = null)
        {
            ...
            if (profileLaunch == false && !runEmuOnly)
            {
                ...
                if (mainWindow != null)
                {
                    mainWindow.menuButton.IsEnabled = false;
                }
            }
            ...
            if (runEmuOnly)
            {
                buttonForceQuit.IsVisible = false;
            }
        ...
        private async void GameRunning_OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_gameProfile.EmulatorType != EmulatorType.OpenParrot)
            {
                if (_gameProfile.EmulationProfile == EmulationProfile.APM3 || _gameProfile.EmulationProfile == EmulationProfile.APM3Direct)
                {
                    ...
                    {
                        await MessageBoxHelper.ErrorOK(TeknoParrotUi.Properties.Resources.ErrorNoAPM3Id);
                        ...
                        else if (_forceQuit == false)
                        {
                            await Dispatcher.UIThread.InvokeAsync(() =>
                            {
                                ...
                                if (mainWindow != null)
                                {
                                    mainWindow.menuButton.IsEnabled = true;
                                }
                            });
                            await Dispatcher.UIThread.InvokeAsync(() =>
                            {
        ...
        private async Task CreateGameProcess()
        {
            ...
            var gameThread = new Thread(() =>
            {
                ...
                cmdProcess.OutputDataReceived += (sender, e) =>
                {
                    ...
                    try
                    {
                        Dispatcher.UIThread.InvokeAsync(() => { textBoxConsole.Text += "\n" + e.Data; });
                    }
                    catch
                    {
                ...
                while (!cmdProcess.HasExited)
                {
                    ...
                    if (_gameProfile.EmulationProfile == EmulationProfile.SegaToolsIDZ)
                    {
                        ...
                        //     // Create a temporary KeyBinding and check if Escape is pressed through it
                        //     var kb = new KeyGesture(Key.Escape);
                        //     isEscapePressed = kb.Matches(null, new KeyEventArgs
                        //     {
                        //         Key = Key.Escape,
        ...
        private void CheckAMDDriver()
        {
            ...
            {
                MessageBoxHelper.WarningOK("Your AMD driver is unsupported for this game. \nIf the game crashes or has new graphical issues, please downgrade to the AMD driver version 22.5.1 or older");
            }
        }
        ...
        {
            // Get the window handle using Avalonia's platform APIs
            var mainWindow = (Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime)?.MainWindow;
            _inputRoot = mainWindow;
            var visual = mainWindow as Visual;
            _windowHandle = IntPtr.Zero;

            if (visual != null)
            {
                var topLevel = TopLevel.GetTopLevel(visual);
                if (topLevel?.PlatformImpl is IPlatformHandle platformHandle)
                {
                    // Use IPlatformHandle to get native handle
            ...
            var inputThread = new Thread(() =>
                InputListener.Listen(
                    Lazydata.ParrotData.UseSto0ZDrivingHack,
                    Lazydata.ParrotData.StoozPercent,
                    _gameProfile.JoystickButtons,
                    _inputApi,
                    _gameProfile
                )
            );
            ...
            // Register for raw input in Avalonia
            // TODO: FIX
            // if (_inputApi == InputApi.RawInput || _inputApi == InputApi.RawInputTrackball)
            // {
            //     // Subscribe to raw input events in Avalonia
            //     var inputManager = AvaloniaLocator.Current.GetService<IInputManager>();
            //     inputManager.Process += OnProcessRawInput;
            // }

            return inputThread;
        }

        private void OnProcessRawInput(object sender, RawInputEventArgs e)
        {
            // TODO: FIX
            // if (_inputApi == InputApi.RawInput || _inputApi == InputApi.RawInputTrackball)
            // {
            //     bool handled = false;
            //     InputListener?.ProcessRawInput(e, ref handled);
