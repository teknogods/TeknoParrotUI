...
7. ⏳ **Platform testing matrix** — Windows SDL2 play-testing, Linux gamepad (`sdl2-test` verified SDL2 init on Linux ✅), Android
8. ✅ **Cleanup (2026-07-09)** — SharpDX fully removed: packages `SharpDX`, `SharpDX.DirectInput`, `SharpDX.XInput`, `SharpDX.RawInput` deleted from the csproj; `InputListenerDirectInput` (3,965 lines) and `XInputDeviceHelper` deleted; `InputListener` reduced to the RawInput/Trackball gun host; SDL2 is the **only** gamepad backend on every platform including Windows (legacy DirectInput/XInput selections resolve to SDL2 and their XInputButton bindings keep working; DirectInput-only bindings are dead — the launch console warns and asks the user to rebind). XInput-shaped state types now live in `InputListening/Gamepad/XInputTypes.cs` (`State`/`XiGamepad`/`GamepadButtonFlags`, same values as XINPUT). Gun/mouse/keyboard input remains Win32 RawInput (`RawInput.Sharp`) on Windows / evdev on Linux / touch on Android.
