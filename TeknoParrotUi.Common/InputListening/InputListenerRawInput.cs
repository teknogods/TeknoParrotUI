...
namespace TeknoParrotUi.Common.InputListening
{
    public class InputListenerRawInput
    {
        ...
        private bool KeyboardAnalogDown = false;
        private bool KeyboardAnalogUp = false;
        private bool KeyboardAnalogXActivate = false;
        private bool KeyboardAnalogYActivate = false;
        private bool ReverseYAxis = false;
        private int KeyboardAnalogXValue = 0x80;
        private int KeyboardAnalogYValue = 0x80;
        private int KeyboardAnalogAxisSensitivity = 10;
        ...
        {
            if ((KeyboardorButtonAxis && joystickButton.HideWithKeyboardForAxis) ||
                (!KeyboardorButtonAxis && joystickButton.HideWithoutKeyboardForAxis))
                return;
            ...
            {
                if (joystickButton.ButtonName.Contains("Left"))
                {
                    KeyboardAnalogLeft = pressed;
                    if (!KeyboardAnalogXActivate)
                        KeyboardAnalogXActivate = true;
                    return;
                }
                else if (joystickButton.ButtonName.Contains("Right"))
                {
                    KeyboardAnalogRight = pressed;
                    if (!KeyboardAnalogXActivate)
                        KeyboardAnalogXActivate = true;
                    return;
                }
            ...
            if (KeyboardorButtonAxis && joystickButton.InputMapping == InputMapping.Analog6)
            {
                if (joystickButton.ButtonName.Contains("Up"))
                {
                    KeyboardAnalogUp = pressed;
                    if (!KeyboardAnalogYActivate)
                    {
                        KeyboardAnalogYActivate = true;
                        // Check if this axis is reversed (AnalogJoystickReverse)
                        ReverseYAxis = joystickButton.AnalogType == AnalogType.AnalogJoystickReverse;
                    }
                    return;
                }
                else if (joystickButton.ButtonName.Contains("Down"))
                {
                    KeyboardAnalogDown = pressed;
                    if (!KeyboardAnalogYActivate)
                    {
                        KeyboardAnalogYActivate = true;
                        // Check if this axis is reversed (AnalogJoystickReverse)
        ...
        private void HandleRawInputGun(JoystickButtons joystickButton, int inputX, int inputY, bool moveAbsolute)
        {
             ...
             * 
             * InvertedMouseAxis:
             * AnalogBytes[0-1] = X (16-bit), Left = 0, Right = 65535
             * AnalogBytes[2-3] = Y (16-bit), Top = 0,  Bottom = 65535
             * 
             * NOT InvertedMouseAxis:
             * AnalogBytes[2-3] = X (16-bit), Left = 65535, Right = 0
             * AnalogBytes[0-1] = Y (16-bit), Top = 65535,  Bottom = 0
             * 
             * Luigi is different ofcourse:
             * AnalogBytes[2-3] = X (16-bit), Left = 0, Right = 65535
             * AnalogBytes[0-1] = Y (16-bit), Top = 0,  Bottom = 65535
             */
