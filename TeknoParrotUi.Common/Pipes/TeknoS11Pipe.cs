...
namespace TeknoParrotUi.Common.Pipes
{
    ...
    public sealed class TeknoS11Pipe : ControlSender
    {
        ...
        private static bool GunProfile() =>
            InputCode.GameProfile?.GunGame == true;
        ...
        public override void Start()
        {
            ...
            // Each driving player has an independent centered wheel lane.
            foreach (var binding in InputCode.GameProfile.JoystickButtons)
            {
                int axis = (int)binding.InputMapping - (int)InputMapping.Analog0;
                if ((binding.AnalogType == AnalogType.Wheel || binding.AnalogType == AnalogType.AnalogJoystick ||
                     binding.AnalogType == AnalogType.AnalogJoystickY) && axis >= 0 && axis < InputCode.AnalogBytes.Length)
                    InputCode.AnalogBytes[axis] = 0x80;
            }
            if (GunProfile())
            {
        ...
        public override void Transmit()
        {
            ...
            byte system = 0;
            // The cabinet test switch is maintained: toggle only on a new
            // press, so release and repeated input polls keep its state.
            var testDown = Down(operatorInput.Test);
            if (testDown && !_testDown)
                _testLatched = !_testLatched;
            _testDown = testDown;
            if (_testLatched) system |= 0x80;
