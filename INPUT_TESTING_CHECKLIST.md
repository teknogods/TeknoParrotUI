...
Current input checklist; CROSSPLATFORM_INPUT_REFACTOR_PLAN.md records the earlier SDL2 rollout.
...
dotnet run --project Tools/InputMethodAudit -- profiles-test   # InputProfile generation for the stock catalog
dotnet run --project Tools/InputMethodAudit -- sdl3-profile-compat-test # legacy saves
dotnet run --project Tools/InputMethodAudit -- sdl3-virtual-test # mapped virtual controller
dotnet run --project Tools/InputMethodAudit -- raw-joystick-test # unmapped virtual controller
dotnet run --project Tools/InputMethodAudit -- sdl3-listener-lifecycle-test # hot-plug and restart
dotnet run --project Tools/InputMethodAudit -- sdl3-capacity-test # more than eight devices
dotnet run --project Tools/InputMethodAudit -- sdl3-test       # live SDL3 gamepad state (15 s)
...
## Windows — SDL3 gamepad

XInput/DirectInput listeners and SharpDX are **removed**. Every gamepad API
selection (including legacy XInput/DirectInput saves) runs the SDL3 listener,
...
### Gamepad (SDL3)
- [ ] Game with legacy `Input API = XInput` save: SDL3 listener starts (console: "gamepads via SDL3"), existing bindings work in-game
...
- [ ] Fresh SDL3 bindings work in-game (buttons, sticks, independent triggers)
- [ ] Existing `SDL2Gamepad` profile JSON and `Input API = SDL2` saves still load and use SDL3
- [ ] `sdl3-virtual-test` detects a mapped virtual gamepad; `raw-joystick-test` detects an unmapped joystick
- [ ] Hot-plug: connect pad after game launch (SDL3 listener picks it up promptly)
...
- [ ] `Input API = RawInput` light gun game (e.g. 2Spicy, Virtua Cop): aim tracks mouse, trigger/buttons fire, crosshair centered at boot; SDL3 gamepad listener runs alongside
...
- [ ] `MergedInput`: SDL3 gamepad + RawInput mouse/keyboard simultaneously
...
### UI
- [ ] Game Settings hides Input API for most games; where both gun modes are supported, it offers only RawInput and RawInputTrackball
- [ ] Multi-game button config modes: Merged Input (Gamepad + Gun) / SDL3 Gamepad / RawInput
- [ ] Per-game bindings view (`JoystickSetupView`) captures via SDL3 for every gamepad selection
- [ ] Controller UI navigation still works (SDL3 capture)
...
### Gamepad (SDL3 — the only gamepad path)
- [ ] `sdl3-test` shows connected pad state (buttons/axes/triggers move)
- [ ] Games with any legacy API selection transparently use SDL3 (log: "not available on this platform, using SDL3")
- [ ] Binding capture works in UI (all modes fall back to SDL3 capture)
- [ ] Bindings persist and match in-game behaviour
- [ ] Hot-plug pad after launch
- [ ] Up to 16 pads simultaneously (slot assignment stable)
...
- [ ] Gun game launch starts EvdevMouse listener alongside SDL3 (RawInput/Merged/Trackball selection or GunGame flag)
...
- [ ] On a physical device, connect a gamepad after TPUI starts: Controller Setup lists it without restarting
- [ ] D-pad keys and hat axes, face/shoulder buttons, sticks, and both triggers bind and work in-game
- [ ] A keyboard-style HID controller that sends Android gamepad button keycodes appears after its first press

## Cross-platform binding compatibility
- [ ] Bindings captured on Windows (XInput mode) work on Linux under SDL3 (same XInputButton storage)
- [ ] Bindings captured on Linux (SDL3) work on Windows in XInput and SDL3 modes
- [ ] UserProfiles XML round-trips across platforms (no serializer differences)

## Sign-off gates
- [x] SharpDX removal — **done 2026-07-09**; SDL3 desktop migration — **2026-09-25**. Android feeds the shared gamepad state from native Android input events. RawInput (Windows), evdev (Linux), and touch (Android) still serve gun games.
