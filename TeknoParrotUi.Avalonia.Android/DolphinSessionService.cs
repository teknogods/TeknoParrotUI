...
/// <summary>Durable foreground owner for one authenticated TeknoDolphin session.</summary>
[Service(
    Name = ServiceClass,
    Exported = false,
    ForegroundServiceType = ForegroundService.TypeSpecialUse)]
public sealed class DolphinSessionService : Service
{
    public const string ServiceClass = "com.teknoparrot.session.DolphinSessionService";
    public const string StartAction = "com.teknoparrot.ui.action.START_DOLPHIN_SESSION";
    public const string StopAction = "com.teknoparrot.ui.action.STOP_DOLPHIN_SESSION";
    public const string ProfileExtra = "com.teknoparrot.ui.extra.DOLPHIN_PROFILE";
    public const string GameIdExtra = "com.teknoparrot.ui.extra.DOLPHIN_GAME_ID";
    public const string GamePathExtra = "com.teknoparrot.ui.extra.DOLPHIN_GAME_PATH";
    public const string TokenExtra = "com.teknoparrot.ui.extra.DOLPHIN_TOKEN";

    private const string CompanionPackage = "com.teknogods.teknodolphin";
    ...
    private const string RemotePrefix = "com.teknoparrot.dolphin";
    ...
    private const string Preferences = "teknoparrot-dolphin-session";
    private const string RecordKey = "active-session-v1";
    private const string ChannelId = "teknoparrot_dolphin_session";
    ...
    public override void OnCreate()
    {
        ...
        manager?.CreateNotificationChannel(new NotificationChannel(
            ChannelId, "TeknoDolphin arcade session", NotificationImportance.Low)
        {
            Description = "Keeps TeknoParrot connected to the running TeknoDolphin game."
        });
...
#pragma warning restore CA1422
        ...
        Intent? intent, StartCommandFlags flags, int startId)
    {
        ...
        try
        {
            StartForegroundSession("Preparing TeknoDolphin arcade session");
            ...
            if (string.Equals(intent?.Action, StartAction, StringComparison.Ordinal))
            {
                ...
                if (saved != null && saved.Token != requested.Token)
                    throw new InvalidOperationException(
                        $"{saved.ProfileName} already owns the TeknoDolphin session.");
                _record = saved ?? requested;
                SaveRecord(_record);
                Publish("state=starting;detail=opening TeknoDolphin");
                Launch(_record);
            }
            else
            {
                ...
                Publish("state=restoring;detail=checking TeknoDolphin session");
    ...
    {
        var intent = new Intent(context, typeof(DolphinSessionService));
    ...
    internal static string? TryRestoreActiveProfileName(Context context)
    {
        ...
        if (CurrentStatus == "state=idle" || CurrentStatus.StartsWith("state=fault"))
            (context.ApplicationContext ?? context).StartForegroundService(
                new Intent(context, typeof(DolphinSessionService)));
        return profile;
    }
    ...
    private static SessionRecord CreateRecord(Intent intent)
    {
        ...
        if (!IsValid(record))
            throw new InvalidDataException("The TeknoDolphin session envelope is invalid.");
        return record;
    }
    ...
    private void Launch(SessionRecord record)
    {
        _ = PackageManager?.GetPackageInfo(CompanionPackage, PackageInfoFlags.Activities)
            ?? throw new InvalidOperationException("TeknoDolphin is not installed.");
        ...
        intent.PutExtra(RemoteInputPage,
            "/storage/emulated/0/Android/data/com.teknogods.teknodolphin/files/" +
    ...
    private void RequestStop()
    {
        ...
        _record = record;
        Publish("state=stopping;detail=requesting TeknoDolphin shutdown");
    ...
    private void HandleStatus(Intent intent)
    {
        ...
        {
            case "accepted":
                Publish("state=accepted;detail=TeknoDolphin accepted the game");
                StartForegroundSession(record.ProfileName + " is starting");
                break;
            case "running":
                Publish("state=running;detail=TeknoDolphin game is active");
                StartForegroundSession(record.ProfileName + " is running");
                break;
            case "stopping":
                Publish("state=stopping;detail=TeknoDolphin is stopping");
                break;
            case "stopped":
                Publish("state=stopped");
                StopHealthMonitor();
    ...
    private void StartHealthMonitor(SessionRecord record)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                while (!cancellation.IsCancellationRequested)
                {
                    ...
                    if (SystemClock.ElapsedRealtime() -
                        ...
                        continue;
                    Publish("state=fault;error=TeknoDolphin is no longer responding");
                    ClearRecord();
                    _record = null;
                    StopForegroundSession();
                    StopSelf();
            ...
            {
                global::Android.Util.Log.Warn("TeknoParrotDolphin", Sanitize(error.Message));
            }
        }, cancellation);
    }
    ...
    private void StartForegroundSession(string detail)
    {
        ...
        var stopIntent = new Intent(this, typeof(DolphinSessionService));
        ...
        var notification = new Notification.Builder(this, ChannelId)
            ...
            .SetContentTitle("TeknoParrot TeknoDolphin session")
...
#pragma warning restore CA1422
    ...
    private sealed class StatusReceiver(DolphinSessionService owner) : BroadcastReceiver
    {
